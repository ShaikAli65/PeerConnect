...
class SimplePeerText:
    ...
    def __init__(self, refer_sock: Socket, data=b'', *, chunk_size=512):
        self.raw_text = data
        self.text_len_packed = _struct.pack('!I', len(self.raw_text))
    ...
    async def send(self, *, require_confirmation=False) -> bool:
        ...
        - _struct.error: If an error occurs while packing the text length.
        Returns:
    ...
    async def receive(self, cmp_string: Union[str, bytes] = '', *, require_confirmation=False) -> Union[bool, bytes]:
        ...
        """
        text_length = _struct.unpack('!I', await self._sock.arecv(4))[0]
...
class DataWeaver:
    ...
    def send(self, receiver_sock):
        ...
        """
        return SimplePeerText(receiver_sock, self.dump().encode(_const.FORMAT)).send()
...
class PeerText(SimplePeerText):
    """
    Defines a Wire Protocol for data transfer between peers
    :todo: complete this as requriments increase
    """